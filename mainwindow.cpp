...
MainWindow::MainWindow(QWidget *parent)//constructor que recibe como parametro QW padre
    : QMainWindow(parent)//Qmain que ejecuta el constructor
...
MainWindow::~MainWindow() //destructor que elimina de la memoria la interfaz grafica usuario (ui)
{
    delete ui;
}
    ...
    consultar.exec(" SELECT* FROM usuarios WHERE usuario ="
       ...
       if(cantidad == 0){
         ...
         if (ui->lineEdit_usuario->text()=="Adm" && control==true){
           //  secDialog secunDialg;//declara una instacia de la clase secDialg creando un objeto segundo dialogo
           //  secunDialg.exec();}
            segundoD = new secDialog (this);
            segundoD->show();}
}
